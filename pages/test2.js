...
export default function Tabs({ color="black", data }) {
  ...
  useEffect(() => {
    ...
    async function web3Balance() {
      ...
      //console.log("Wallet address in OM: ", ostate.user.wallets.address)
      const result = await Wallet.getTokenBalance("TING", '0x489e4CFfa9B59784C597C51cd24000b1db506c20');
      console.log(result);
    }
    web3Balance();    
...
export async function getStaticProps() {
  const data = await Wallet.getTokenData();
  return { props: { data } };
}

{/*
Included as part of tabs color customization.

  export default function TabsRender(res) {
    return (
      <div>
        <Tabs color="black" result={res} />;
      </div>
    );
  }

Also fetches data from api except only runs on build, I think. Similar to getServerSideProps. What's best?

  export async function getStaticProps() {
    const res = await fetch("https://api.tryroll.com/v1/tokens/exchange");
...
Deprecated?
  ...
  Tabs.getInitialProps = async () => {
    const req = await fetch("https://api.tryroll.com/v1/tokens/exchange");
